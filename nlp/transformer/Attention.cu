...
/* 默认输入输出都为device端 */
void Attention::forward(MatrixHalf *inputQ, MatrixHalf *inputK, MatrixHalf *inputV, MatrixHalf *output, int layer, int which_part, int *mask) {
...
void Attention::attn_batch(half *Q, half *K, half *V, half *out, int batch, int en_max_len, int de_max_len, int *mask) {
    ...
    // 5. 和V乘
    //sparse_mma_gemm_noPad_batches_device(softmax_out, V, batch * heads, de_max_len, en_max_len, embedding / heads, false, out);
