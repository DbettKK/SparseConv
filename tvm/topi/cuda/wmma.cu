...
#endif
extern "C" __global__ void __launch_bounds__(32) default_function_kernel0(float* __restrict__ Data, float* __restrict__ Filter, float* __restrict__ Conv2dOutput) {
  nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float> Conv2dOutput_wmma_accumulator[1];
  ...
  __shared__ float Conv2dOutput_wmma_accumulator_shared[256];
  ...
  for (int ry = 0; ry < 4; ++ry) {
    for (int rx = 0; rx < 4; ++rx) {
      ...
      for (int ax0_ax3_fused_outer_outer_outer_outer = 0; ax0_ax3_fused_outer_outer_outer_outer < 8; ++ax0_ax3_fused_outer_outer_outer_outer) {
        compute_shared[(((ax0_ax3_fused_outer_outer_outer_outer * 32) + ((int)threadIdx.x)))] = ((half)Data[((((((((ax0_ax3_fused_outer_outer_outer_outer * 1568) + ((((int)threadIdx.x) >> 4) * 784)) + ((((int)blockIdx.z) >> 2) * 112)) + (ry * 112)) + (rx * 16)) + ((((int)blockIdx.z) & 3) * 16)) + (((int)threadIdx.x) & 15)))]);
      }
      for (int ax2_ax3_fused_outer_outer_outer_outer = 0; ax2_ax3_fused_outer_outer_outer_outer < 8; ++ax2_ax3_fused_outer_outer_outer_outer) {
        compute_d_shared[(((ax2_ax3_fused_outer_outer_outer_outer * 32) + ((int)threadIdx.x)))] = ((half)Filter[(((((ry * 1024) + (rx * 256)) + (ax2_ax3_fused_outer_outer_outer_outer * 32)) + ((int)threadIdx.x)))]);
      }
      __syncthreads();
  ...
  (void)nvcuda::wmma::store_matrix_sync(((float *)Conv2dOutput_wmma_accumulator_shared + (0)), Conv2dOutput_wmma_accumulator[0], 16, nvcuda::wmma::mem_row_major);
  __syncthreads();
  for (int nn_inner_ff_inner_fused_outer_outer_outer_outer = 0; nn_inner_ff_inner_fused_outer_outer_outer_outer < 8; ++nn_inner_ff_inner_fused_outer_outer_outer_outer) {
    Conv2dOutput[(((((nn_inner_ff_inner_fused_outer_outer_outer_outer * 512) + ((((int)threadIdx.x) >> 4) * 256)) + (((int)blockIdx.z) * 16)) + (((int)threadIdx.x) & 15)))] = Conv2dOutput_wmma_accumulator_shared[(((nn_inner_ff_inner_fused_outer_outer_outer_outer * 32) + ((int)threadIdx.x)))];
  }
}
